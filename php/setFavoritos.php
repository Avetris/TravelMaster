	...
	if(isset($_POST["auth_token"]) && isset($_POST["id_dispositivo"]) && isset($_POST["datos"])){
		...
		if ($stmt->execute()) {
			...
			if($stmt->fetch() && $count > 0){
				$stmt->close();
				$stmt2 = $conn->prepare("REPLACE INTO FAVOURITES (FAVOURITES_USER_ID, FAVOURITES_PLACE_ID, FAVOURITES_STATE, FAVOURITES_FECHAMOD) VALUES (?,?,?,SYSDATE())");
				$stmt2->bind_param("iii", $datos -> {"id_usuario"}, $datos -> {"id_lugar"}, $datos -> {"estado_favorito"});
				if($stmt2->execute()){
					$stmt2->fetch();
		  			$stmt2->close();
					$arrayFila = [];
					...
					$arrayFila["estado"] = $datos -> {"estado_favorito"};
					$miArray[] = $arrayFila;
				}else{
		  			$error = $stmt2->error; 
		  			$stmt2->close();
