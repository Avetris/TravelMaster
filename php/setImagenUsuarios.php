	...
	if(isset($_POST["auth_token"]) && isset($_POST["id_dispositivo"]) && isset($_POST["datos"])){
		...
		if ($stmt->execute()) {
			...
			if($stmt->fetch() && $count > 0){
				$stmt->close();
				$ruta = "img/usuarios/".str_replace("-", "_", $id_dispositivo).getdate()[0].".jpg";
				$fp = fopen($ruta, 'w');
				$ruta = "http://galan.ehu.eus/umateos002/WEB/TravelMaster/".$ruta;
				fwrite($fp, base64_decode($datos -> {"imagen_usuario"}));
				if(fclose($fp)){
					$stmt2 = $conn->prepare("UPDATE USER SET USER_IMAGE = ?, USER_FECHAMOD = SYSDATE() WHERE USER_ID = ?");
					$stmt2->bind_param("si", $ruta, $datos -> {"id_usuario"});
					if($stmt2->execute()){
						$stmt2->fetch();
			  			$stmt2->close();
						$arrayFila = [];
						$arrayFila["id_usuario"] = $datos -> {"id_usuario"};
						$arrayFila["imagen_usuario"] = $ruta;				
						$miArray[] = $arrayFila;
					}else{
			  			$error = $stmt2->error; 
			  			$stmt2->close();
