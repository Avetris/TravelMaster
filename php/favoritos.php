<?php 
	...
	if(isset($_POST["auth_token"]) && isset($_POST["id_dispositivo"])){
		...
		$sql = "SELECT FAVOURITES_USER_ID, FAVOURITES_PLACE_ID, FAVOURITES_STATE FROM FAVOURITES";
		if(count($date) != 0){
			$sql = $sql." WHERE FAVOURITES_FECHAMOD >= ?";		
		}
		...
		if ($stmt->execute()) {
			...
			if($stmt->fetch() && $count > 0){
				...
				$stmt->execute();
				$stmt->bind_result($user_id, $place_id, $place_state);	
				$miArray = [];
				while($stmt->fetch()){
					$arrayFila = [];
					$arrayFila["id_usuario"] = $user_id;
					$arrayFila["id_lugar"] = $place_id;
					$arrayFila["estado"] = $place_state;
					$miArray[] = $arrayFila;
    			}
			}else{
				$stmt->close();
