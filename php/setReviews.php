	...
	if(isset($_POST["auth_token"]) && isset($_POST["id_dispositivo"]) && isset($_POST["datos"])){
		...
		if ($stmt->execute()) {
			...
			if($stmt->fetch() && $count > 0){
				$stmt->close();
				$stmt2 = $conn->prepare("INSERT INTO REVIEW (REVIEW_USER_ID, REVIEW_PLACE_ID, REVIEW_TITLE, REVIEW_DESCRIPTION, REVIEW_RATING, REVIEW_FECHAMOD) VALUES(?,?,?,?,?,SYSDATE())");
				$stmt2->bind_param("iissi", $datos -> {"id_usuario"}, $datos -> {"id_lugar"}, $datos -> {"titulo_review"}, $datos -> {"descripcion_review"}, $datos -> {"valoracion_review"});
				if($stmt2->execute()){
					$stmt2->fetch();
		  			$stmt2->close();
					$arrayFila = [];
					$arrayFila["id_usuario"] = $datos -> {"id_usuario"};
					$arrayFila["id_lugar"] = $datos -> {"id_lugar"};
					$arrayFila["titulo_review"] = $datos -> {"titulo_review"};
					$arrayFila["descripcion_review"] =  $datos -> {"descripcion_review"};
					$arrayFila["valoracion_review"] = $datos -> {"valoracion_review"};
					$miArray[] = $arrayFila;
				}else{
		  			$error = $stmt2->error; 
		  			$stmt2->close();
