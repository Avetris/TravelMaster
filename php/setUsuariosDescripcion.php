	...
	if(isset($_POST["auth_token"]) && isset($_POST["id_dispositivo"]) && isset($_POST["datos"])){
		...
		if ($stmt->execute()) {
			...
			if($stmt->fetch() && $count > 0){
				$stmt->close();
				$stmt2 = $conn->prepare("UPDATE USER SET USER_DESCRIPTION = ?, USER_FECHAMOD = SYSDATE() WHERE USER_ID = ?");
				$stmt2->bind_param("si", $datos -> {"descripcion_usuario"}, $datos -> {"id_usuario"});
				if($stmt2->execute()){
					$stmt2->fetch();
		  			$stmt2->close();
					$stmt->close();
					...
					$arrayFila["descripcion_usuario"] = $datos -> {"descripcion_usuario"};			
			  		$miArray[] = $arrayFila;
				}else{
		  			$error = $stmt2->error; 
		  			$stmt2->close();
