...
public class ListaLugaresFragment extends Fragment {
    ...
    public ListaLugaresFragment() {
        // Required empty public constructor
    }

    public static ListaLugaresFragment newInstance(int idUsuario) {
        ListaLugaresFragment fragment = new ListaLugaresFragment();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            usuario = realm.where(Usuario.class).equalTo("idUsuario", getArguments().getInt("idUsuario")).findFirst();
        }
    }
    ...
    private void recogerDatosBusqueda(String busqueda){
        datos.clear();
        RealmQuery<Lugar> query = realm.where(Lugar.class).contains("nombreLugar",busqueda, RealmQuery.CASE_INSENSITIVE).equalTo("idCreador", usuario.getIdUsuario());
    ...
    private void recogerDatosIniciales(){
        datos.clear();
        RealmQuery<Lugar> query = realm.where(Lugar.class).equalTo("idCreador", usuario.getIdUsuario());
        if(categorias.size() > 0){
            boolean primero = true;
            query = query.beginGroup();
            for(String categoria : categorias){
    ...
    public void onAttach(Activity activity) {
        ...
        if (activity instanceof ListaLugaresFragment.OnFragmentInteractionListener) {
            mListener = (ListaLugaresFragment.OnFragmentInteractionListener) activity;
        } else {
